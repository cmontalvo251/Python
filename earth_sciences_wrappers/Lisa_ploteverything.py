...
def do_everything(FILES,sigma_pitot,numPitots,CAL_TIMES,sigma_anemometer,SHOWPLOTS):
    ...
    #pltALT.legend(loc='lower left')
    lgd=pltALT.legend(loc='center left', bbox_to_anchor=(-0.55, 0.5))
    ...
    if PITOT:
        ...
        for idx in range(0,PITOT):
            ...
            if numPitots == 4:
                ...
                #pN.plot(time_pitot_gps,-northwindspeed_pitot,label='FASTPitot North')
                #pN.plot(time_pitot_gps,-southwindspeed_pitot,label='FASTPitot South')
                
                #Pitot with 1 minute average
                avg_data = M.averages(time_pitot_gps[idx],northwindspeed_pitot_i,1)
                ...
                avg_data = M.averages(time_pitot_gps[idx],southwindspeed_pitot_i,1)
                time_pitot_avg = avg_data[0]
    ...
    if PITOT:
        #Plot east component
        #plot3.plot(time_pitot_gps,eastwindspeed_pitot,label='FASTPitot Raw')
        
        #Pitot with one minute average
        for idx in range(0,PITOT):
            avg_data = M.averages(time_pitot_gps[idx],eastwindspeed_pitot[idx],1)
            time_pitot_avg = avg_data[0]
            ...
            #plot west component
            if numPitots == 4:
                #plot3.plot(time_pitot_gps,-westwindspeed_pitot,label='FASTPitot West')
                
                #1 minute averages
                avg_data = M.averages(time_pitot_gps[idx],westwindspeed_pitot[idx],1)
                time_pitot_avg = avg_data[0]
    ...
    try:
        ...
        avg_data = M.averages(time_anemometer,wind_anemometer,1)
        p.plot(avg_data[0],avg_data[1],color=anem_color,marker='o',label='Anemometer')
    except:
    ...
    if numPitots == 4 and PITOT:
        for idx in range(0,PITOT):
            windspeed_pitot = np.sqrt(southwindspeed_pitot[idx]**2 + northwindspeed_pitot[idx]**2 + westwindspeed_pitot[idx]**2 + eastwindspeed_pitot[idx]**2)
            #p.plot(time_pitot_gps,windspeed_pitot,label='FASTPitot')
            avg_data = M.averages(time_pitot_gps[idx],windspeed_pitot,1)
            p.plot(avg_data[0],avg_data[1],marker=mes_marker[0],color=pitot_color[idx],label='FASTPitot '+str(idx)+' One Minute Average')
    ...
    if PITOT:
        ...
        for idx in range(0,PITOT):
            p.plot(eastwindspeed_pitot[idx],altitude_pitot[idx],marker='s',color='c',label=idx)
        p.set_ylabel('Altitude (m)')
        p.set_xlabel('WindSpeed (m/s)')
    ...
    if IMET or QUAD or PTH_PITOT or PTH_MESONET or PTH_ANEMOMETER:
        ...
        if PTH_PITOT or PTH_ANEMOMETER or PTH_MESONET:
            ...
            if PTH_PITOT:
                for idx in range(0,PITOT):
                    data_pitot_PTH_i = data_pitot_PTH[idx]
                    temperature_pitot = data_pitot_PTH_i[1]
                    pC.plot(time_pitot_gps[idx],temperature_pitot,color=pitot_color[idx],label='FASTPitot '+str(idx))
            if PTH_ANEMOMETER:
            ...
            if PTH_PITOT:
                for idx in range(0,PITOT):
                    data_pitot_PTH_i = data_pitot_PTH[idx]
                    humidity_pitot = data_pitot_PTH_i[3]
                    pH.plot(time_pitot_gps[idx],humidity_pitot,color=pitot_color[idx],label='FASTPitot '+str(idx))
            if PTH_ANEMOMETER:
