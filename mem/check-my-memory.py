...
print (sys.argv[0], "- version: SJ 2013-02-22")
...
if os.popen('whoami').readline().strip() == 'root' :
	...
	print "Without root rights, you get less (or less correct) information.\n\n"
	# sys.exit(1)
	root = False
...
for thisline in os.popen(cmd).readlines():
	...
	if thisline.find("mem") > -1 :
		# something with mem
                ...
                ###Search for [mem
                ctr = 0
                this_element = row[ctr]
                while row[ctr] != '[mem':
                        print row[ctr]
                        ctr+=1
                hexstuff = row[ctr+1].rstrip(']')
                print ('row = ',row)
                print ('hexstuff = ',hexstuff)
