...
class Vehicle():
    ...
    def ForceMoment(self,t,state,xyzdot):
        ...
        Iinf = self.Iinv
        Fu = fjkdlsafjlkds
        Gu = np.asarray([0,(1.0/m)*0.5*Roe*(vinf**2)*CLdele*np.sin(alpha),0,(1.0/m)])
        gv = jfkdlfjdlks
        Gv = np.asarray([0,0,gv,0])
        H2 = fkjdlfjdkl;fdjlk
        BIGMAT = fjkdls;fjkdsl;fjkl
        Ga = 0.5*Roe*(vinf**2)*S*np.matmul(H2,np.matmul(invI,BIGMAT))
        Fa = fjdklfjdlksfjlkds

        G = np.concatenate(Gu,Gv,Ga)
        F = np.concatenate(Fu,Fv,Fa)

        Ginv = np.linalg.inv(G)

        Gamma = np.asarray([-kp*(u-uc),fjkdlfjdkl])
        
        U = np.matmul(Ginv,Gamma-F)
